...
def save_sp500_tickers():
    ...
    for faslcount, fasl in enumerate(fasls): #range(3,4)
        faslcount=faslcount+1
        Num=0
        for url in fasls[fasl]: #[ "فصل سوم دعاهای ایام هفته‌ "]:
            ...
            for e in soup(['figure', 'script']): e.decompose()
            ...
            About = soup.findAll('article', {'class': 'js_AboutText'}) #js_MafatidText MafatidText m0a table
            sects=(str(About)).split(",")
            for sec in sects:

                row= sec.replace('<article class="js_AboutText AboutText Translate">', '')
                # print(sec)
                # break
                row=row.replace('<', '')
                row=row.replace('>', '')
                row=row.replace('[', '')
                row=row.replace(']', '')
                row=row.replace('</article>]', '')
                row=row.replace('article', '')
                # if row=='': row=[]
                row = row.replace('/', '')
                tocount=tocount+1
                Tozih.update({tocount:row})

            Tar = soup.findAll('article', {'class': 'js_TranslateText'}) #js_MafatidText MafatidText m0a table
            sects=(str(Tar)).split(",")
            for sec in sects:
                row= sec.replace('<article class="js_TranslateText TranslateText Translate">', '')
                row=row.replace('<', '')
                row=row.replace('>', '')
                row=row.replace('[', '')
                row=row.replace(']', '')
                ...
                # # article_soup = [e.get_text() for e in sects]  #gets all the text and its great
                # arcount=arcount+1
                # Arabics.update({arcount:article_soup})
            sects=(str(ArabTar).split('article'))
            # print(sects[1])
            # break
            for sec in sects:
            ...
            Num=Num+1
            data={Num:{'number': Num , 'fasl': fasl ,'title' :Title ,  'arabic': Arabics , 'farsi': Farsi, 'tozih': Tozih , }}

            dirDestination="G:\\Flutter\\Qurani2\\python"
            filename=f"{Num}"

            # fasl= "فصل سوم دعاهای ایام هفته‌ "
            dirr='{}\\{}'.format(dirDestination,faslcount)
